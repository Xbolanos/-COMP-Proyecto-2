#include < stdio . h > #include < stdlib . h > #include < string . h > #include < mpi . h > #define MAX_CHAR_SIZE 2048 
...
void needlemanWunsch  (char h1 [ ] , char h2 [ ] , int rank , int size , int blockA ) 
  {
    ...
    if  (rank == 0 ) 
    ...
    else 
      {
        ...
        i < length2 ;
        i ++ ) F [ 0 ] [ i ] = - j ++ ;
        if  (rank <  (size - 1 ) ) MPI_Send  (& j , sizeof  (j ) , MPI_INT ,  (rank + 1 ) , 0 , MPI_COMM_WORLD ) ;
        for  (i = 1 ;
        i < length1 ;
        i += blockA ) 
          {
        ...
        if  (rank ==  (size - 1 ) ) printf  ("El resultado es: %d.\n\n" , F [ length1 - 1 ] [ length2 - 1 ] ) ;
      }
      }
